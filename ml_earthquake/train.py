...
def train(
    ...
    model.add(ConvLSTM2D(
        input_shape=(X_train.shape[1] - 2, X_train.shape[2] -
                     2, X_train.shape[3] - 2, X_train.shape[4]),
        filters=30,
        kernel_size=(3, 3),
        padding='same',
        dropout=dropout,
    ...
    model.add(ConvLSTM2D(
        input_shape=(X_train.shape[1] - 4, X_train.shape[2] -
                     4, X_train.shape[3] - 4, X_train.shape[4]),
        filters=30,
        kernel_size=(3, 3),
        padding='same',
        dropout=dropout,
    ...
    #     kernel_size,
    #     activation='relu',
    #     input_shape=(X_train.shape[1] - (kernel_size - 1), X_train.shape[2] - (
    #         kernel_size - 1), X_train.shape[3] - (kernel_size - 1), filters),
    # ))
    # model.add(BatchNormalization())
    ...
    #     kernel_size,
    #     activation='relu',
    #     input_shape=(X_train.shape[1] - 2 * (kernel_size - 1), X_train.shape[2] - 2 * (
    #         kernel_size - 1), X_train.shape[3] - 2 * (kernel_size - 1), filters),
    # ))
    # model.add(BatchNormalization())
    ...
    #     kernel_size,
    #     activation='relu',
    #     input_shape=(X_train.shape[1] - 3 * (kernel_size - 1), X_train.shape[2] - 3 * (
    #         kernel_size - 1), X_train.shape[3] - 3 * (kernel_size - 1), filters),
    # ))
    # model.add(BatchNormalization())
    ...
    #     kernel_size,
    #     activation='relu',
    #     input_shape=(X_train.shape[1] - 4 * (kernel_size - 1), X_train.shape[2] - 4 * (
    #         kernel_size - 1), X_train.shape[3] - 4 * (kernel_size - 1), filters),
    # ))
    # model.add(BatchNormalization())
    ...
    #     kernel_size,
    #     activation='relu',
    #     input_shape=(X_train.shape[1] - 5 * (kernel_size - 1), X_train.shape[2] - 5 * (
    #         kernel_size - 1), X_train.shape[3] - 5 * (kernel_size - 1), filters),
    # ))
    # model.add(BatchNormalization())
    ...
    #     kernel_size,
    #     activation='relu',
    #     input_shape=(X_train.shape[1] - 6 * (kernel_size - 1), X_train.shape[2] - 6 * (
    #         kernel_size - 1), X_train.shape[3] - 6 * (kernel_size - 1), filters),
    # ))
    # model.add(BatchNormalization())
    ...
    if log_dir is not None:
        log_path = os.path.join(
            log_dir, datetime.now().strftime(date_format + '_%H%M%S'))
    ...
    if out_dir is not None and info_train is not None and info_test is not None:
        _output(out_dir, X_test, y_test, info_train,
                info_test, model_path, reporter)


def _output(out_dir, X_test, y_test, info_train, info_test, model_path, reporter):
    best_model = load_model(model_path)
    acc, auc, f1, precision, recall, tp, fn, fp, tn = _eval(
    ...
    _dump({'validations': validations},
          os.path.join(out_dir, 'validations.json'))
...
def _resample(X_train, y_train, resampling_method, random_state):
    ...
    X_train_resample, y_train = resampler.fit_sample(
