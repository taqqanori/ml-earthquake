...
def xgb(col):
    ...
    swarms = pd.read_csv('data/swarms_table.csv')
    sigs = pd.read_csv('data/significants_table.csv')

    swarms_X = swarms
    sigs_X = sigs
    for y_label in y_labels:
        swarms_X = swarms_X.drop(y_label, axis=1)
        sigs_X = sigs_X.drop(y_label, axis=1)
    swarms_X = swarms_X.add_prefix('swarm ')
    sigs_X = sigs_X.add_prefix('significant ')
    X = pd.concat([swarms_X, sigs_X], axis=1)
